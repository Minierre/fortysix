...
const { generateRandomNumbers, sumRandomNumbers } = require('../modules/randomLargeSum')
...
// hack to handle room not existing when they press start
const getRoom = (object = {}) => object || {}
...
module.exports = (io) => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('done', ({
      ...
      if (taskExists) {
        rooms[room].tasks = remove(rooms[room].tasks, task => task.id !== id)
      }
      if (rooms[room].tasks.length > 0) {
        rooms[room].nodes[socket.id].running = true
        io.sockets.sockets[socket.id].emit(`CALL_${room}`, rooms[room].tasks[0], graph, {
      ...
      if (allDone && rooms[room].jobRunning) {
        ...
        console.log(chalk.green(`DURATION OF ${room}: `, endTime - rooms[room].start))

        console.log(chalk.magenta(`FINAL RESULT ${finalResult.tour} ${finalResult.dist}`))
  ...
  socket.on(startName, (args) => {
    ...
    if (rooms[room]) {
      if (!rooms[room].running) {
        ...
        partition(io, room, args, (portions) => {
